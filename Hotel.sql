...
VALUES
    ...
    ('Шведский стол','Буфет в ресторане отеля',            800.00),
    ...
    ('Трансфер','Комфортабельный автомобиль',       2000.00),
...
FROM Customer c, Service s
WHERE c.FullName = 'Иванов Иван Иванович' AND s.ServiceName = 'Шведский стол'
UNION ALL
SELECT
...
FROM Customer c, Service s
WHERE c.FullName = 'Сидоров Павел Николаевич' AND s.ServiceName = 'Трансфер';
...
FROM Room r, Service s
WHERE r.RoomNumber IN ('201','202','203','302','303','401') AND s.ServiceName = 'Шведский стол'
UNION ALL
SELECT
    r.$node_id, s.$node_id, '2025-01-01', 0.00
FROM Room r, Service s
...
-- Найти все услуги, доступные клиенту
SELECT 
    c.FullName     AS Начальная_точка_Клиент,
    s.ServiceName  AS Конечная_точка_Услуга
FROM 
    Customer c,
    Booking b,
    Room r,
...
ORDER BY 
    c.FullName, s.ServiceName, r.RoomNumber;
